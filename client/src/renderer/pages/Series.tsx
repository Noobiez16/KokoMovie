...
export function SeriesPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['series', activeProfile.id, page],
    queryFn: () => catalogApi.browse({ type: 'series', limit: 40, page }, activeProfile.id),
    staleTime: 5 * 60 * 1000,
  })
    ...
    <AppLayout>
      <div className="px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-bold text-white">TV Shows & Series</h1>
          {totalPages > 1 && (
        ...
        {isError && (
          <div className="text-purple-300/40 py-32 text-center text-sm">Failed to load series.</div>
        )}
