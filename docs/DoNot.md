...
**DO NOT:**
- Re-introduce a blocking "collect all alternatives" wait in the default Fast & Progressive mode — it puts the whole collection latency in front of playback. Resolve first, collect in the background, and push live status events. The explicit user-selected Complete Scan mode is the only exception and remains bounded by the 40-second hard deadline.
